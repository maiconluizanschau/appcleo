...
export class ConfiguracaoService extends BaseService {
  ...
  public buscaRelacaoConfigMostrar(): FirebaseObjectObservable<Config> {
    return <FirebaseObjectObservable<Config>>this.db.object(`/config/${this.authService.getUid()}`)
    }
  ...
  public gravaInteresse(val: boolean, genero: string) {
    ...
    let refeConfigConversa = this.db.database.ref(`/conversa/${this.idUser}`);
    if (genero == "Mas") {
      if (val === true) {
        console.log("H true");
        refeConfig.update(
    ...
    else if (genero == "relacao") {
      if (val === true) {
        console.log("Relação true");
        refeConfig.update(
          {
            relacaoSerBuscado: true,
          }
        )
        refeConfigConversa.update(
          {
