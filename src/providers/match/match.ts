...
export class MatchService extends BaseService {
  ...
  //CRIA GRUPO
  public createFlertLikes2(grupo: string, userId1: string, tipo: boolean): firebase.Promise<void> {
    if (tipo) {
      this.getVoto(grupo)//busca votos totais
        .then(total => {
          this.setNewVotos(grupo, total.val()); //incrmeneta voto
        });
    }
    ...
    this.getView(grupo)
      .then(total => {
        this.setNewView(grupo, total.val()); //incrmeneta view
      });

      let refeGroup = this.db.database.ref(`/membros/${grupo}/${userId1}`);
  
         return refeGroup.update(
            {
              key: this.user,
              voto: tipo
            }
  ...
  //BUSCAR VOTO DE USER ESPECIFICO
  buscarVotoUserEspecifico(userMatch: string): FirebaseObjectObservable<Voto>{
    let minhakey = this.authService.getUid();
    return <FirebaseObjectObservable<Voto>>this.db.object(`/voto/${userMatch}/${minhakey}`)
  ...
  public getVotos(id: string): FirebaseListObservable<Voto[]> {
   ...
   //BUSCA VOTOS DO ID ESPECIFICO
   public getVotosTrue(id: string): FirebaseListObservable<Voto[]> {
    return <FirebaseListObservable<Voto[]>>this.db.list(`/voto/${id}`, {
      query: {
        orderByChild: 'voto', // filtrando pelo campo "voto"
        equalTo: true, // e que tnha o valor true
  ...
  public getSecretsIds(id: string): FirebaseListObservable<any[]> {
    return <FirebaseListObservable<any[]>>this.db.list(`/votoSecret/${id}`)
      .catch(this.handleObservableError);
  }
  ...
  public remover(key, minhakey) {
    ...
    firebase.database().ref("chats/" + minhakey + "/" + key).remove();
    firebase.database().ref("chats/" + key + "/" + minhakey).remove();
  }
